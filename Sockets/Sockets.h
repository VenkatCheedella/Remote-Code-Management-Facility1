...
public:
  ...
  bool recvBlock(size_t bytes, byte* buffer);
  bool sendblock(byte* buffer, size_t bytesLeft);
...
bool SocketListener::start(CallObj& co)
{
  ...
  std::thread ListenThread(
    [&]()
    {
      ...
      while (!acceptFailed_)
      {
        if (stop_.load())								// What is stop_.load() does? What is its role?
          break;

        // Accept a client socket - blocking call

        Socket clientSocket = accept();    // uses move ctor             // Accept() doesn't have client socket information, how the client socket is returned?
  ...
  std::cout << "\n" << "Listen thread is detached" << endl;
  ListenThread.detach();
  return true;
}
