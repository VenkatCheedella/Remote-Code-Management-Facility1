...
bool Socket::sendblock(byte* buffer, size_t bytesLeft)
{
	byte* pBuf = buffer;
	while (true)
	{
		
		size_t bytesSent = ::send(socket_, pBuf, bytesLeft, 0);
		//ApplicationHelpers::putLine(std::to_string(bytesSent));
...
bool Socket::recvBlock(size_t bytes, byte* buffer)
{
...
bool Socket::recv(size_t bytes, byte* buffer)
{
  ...
  while (true)
  {
    ...
    if (socket_ == INVALID_SOCKET || bytesRecvd == 0)
      return false;
	string dfile(pBuf);
	dfile = dfile.substr(0, 1024);
	outfile.append(dfile);	
	dfile.empty();
	if (bytesRecvd < 1024)
		break;
	//std::cout << "\n" <<dfile;
...
bool SocketConnecter::connect(const std::string& ip, size_t port)
{
  ...
  for (ptr = result; ptr != NULL; ptr = ptr->ai_next) {
	...
	std::cout << "\n" << "Client try to connect through " << ipver << ":" << ipstr;
    //printf("\n  Client try to connect through %s: %s", ipver, ipstr);
...
public:
  ...
  bool testBlockHandling(Socket& socket_);
