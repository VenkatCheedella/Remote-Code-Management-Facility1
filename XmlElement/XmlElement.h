...
// Author:      venkata subba rao cheedella                      //
//              vcheedel@syr.edu                                 //
///////////////////////////////////////////////////////////////////
/*
* Package Operations:
...
* --------------------
*ver 1.6 : 23 Mar 15
*- toString for procInstruction elemet, XmlDeclaration element has been implemented
*- missing few functionalities has been included
* ver 1.5 : 22 Feb 15
...
namespace XmlProcessing
{
  ...
  public:
	...
	DocElement(DocElement&& docElem) :children_(std::move(docElem.children_)){}
	DocElement& operator=(DocElement&& docElem);
    ~DocElement() {}
	...
	std::vector<std::shared_ptr<AbstractXmlElement>>& getChildern() { return children_; }
  private:
    bool hasXmlRoot();
  ...
  public:
	  TextElement(){}
	  TextElement(TextElement&& te) :text_(std::move(te.text_)){}
	  TextElement& operator=(TextElement&& te);
  ...
  {
  public:
	TaggedElement() :tag_("default"){}
    TaggedElement(const std::string& tag) : tag_(tag) {};
	TaggedElement(TaggedElement&& tagElem);
	TaggedElement& operator=(TaggedElement&& te);
    ...
    virtual std::string toString();
	std::vector<std::shared_ptr<AbstractXmlElement>>& getChildern() { return children_; }
	std::vector<std::pair<std::string, std::string>>& getAttributes() { return attribs_; }
  private:
    std::string tag_;
    std::vector<std::shared_ptr<AbstractXmlElement>> children_;
  ...
  public:
	  CommentElement(){}
	  CommentElement(CommentElement&& comElem) :commentText_(std::move(commentText_)){}
	  CommentElement& operator=(CommentElement&& commElem);
  ...
  inline std::string CommentElement::toString()
  {
  ...
  public:
	  ProcInstrElement(){}
	  ProcInstrElement(ProcInstrElement&& procElem) : type_(procElem.type_), attribs_(procElem.attribs_){ procElem.type_ = ""; procElem.attribs_.clear(); }
	  ProcInstrElement& operator=(ProcInstrElement&& procElem);
	...
	virtual std::string toString();
	std::vector<std::pair<std::string, std::string>>& getAttribs() { return attribs_; }
  private:
    std::vector<std::pair<std::string, std::string>> attribs_;
    std::string type_;
    std::string text;
  ...
  public:
	...
	XmlDeclarElement(){}
	XmlDeclarElement(XmlDeclarElement&& xmlDec) :attribs_(std::move(xmlDec.attribs_)){}
	XmlDeclarElement& operator=(XmlDeclarElement&& xmlDec);
    //XmlDeclarElement() {}
  ...
  inline XmlDeclarElement&  XmlDeclarElement::operator=(XmlDeclarElement&& xmlDec)
  {
	  if (this == &xmlDec) return *this;
	  attribs_ = std::move(xmlDec.attribs_);
	  return *this;
  }
